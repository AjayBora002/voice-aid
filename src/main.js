...
function getClaudeKey() { 
  const envKey = typeof import.meta !== 'undefined' && import.meta.env ? import.meta.env.VITE_CLAUDE_API_KEY : '';
  return envKey || localStorage.getItem(CLAUDE_KEY_STORAGE) || ''; 
}
...
window.generateSituationReport = async function() {
  ...
  if (!apiKey) {
    content.innerHTML = `<div style="background:rgba(245,158,11,0.1);border:1px solid rgba(245,158,11,0.3);border-radius:8px;padding:14px;font-size:13px;color:#f59e0b">
      ⚠️ No Claude API key found in Environment Variables. Add VITE_CLAUDE_API_KEY to your Vercel project settings to enable this feature.
    </div>`;
    return;
  }
  btn.disabled = true;
...
const TOUR_STEPS = [
  ...
  { title: '🤖 Claude AI Insights', desc: 'Click "Generate AI Situation Report" to get a real-time commander briefing powered by Claude AI. Add your API key in Settings first.', position: { top: '50%', left: '50%', transform: 'translate(-50%, -50%)' } }
];
